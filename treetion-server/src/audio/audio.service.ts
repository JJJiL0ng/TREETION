...
export class AudioService {
    ...
    async create(
        ...
        try {
            ...
            // STT+GPT 처리 먼저 시작
            sttPromise = this.sttService.processAudioWithSttAndGpt(
                file,
                userId,
                createAudioDto.language || 'ko'
            ).catch(error => {
                this.logger.error(`STT+GPT 처리 중 오류 발생, 계속 진행: ${error.message}`, error.stack);
            ...
            uploadPromise = new Promise(resolve => {
                setTimeout(async () => {
                    try {
                        const result = await this.uploadFileToR2(file.path, fileKey, file.mimetype);
                        resolve(result);
                    } catch (error) {
                        this.logger.error(`R2 업로드 중 오류: ${error.message}`, error.stack);
            ...
            // 병렬 처리 결과 대기
            const [sttResult, uploadResult] = await Promise.all([sttPromise, uploadPromise]);

            // 3. R2 업로드 결과 검증
            if (!uploadResult) {
                throw new InternalServerErrorException('오디오 파일 업로드에 실패했습니다.');
            }
            ...
            // STT+GPT 결과가 있으면 추가
            if (sttResult) {
                this.logger.log(`STT+GPT 처리 결과 통합: 원본 텍스트 길이=${sttResult.originalText.length}, 향상 텍스트 길이=${sttResult.enhancedText.length}`);
    ...
    async upgradeAudioStt(audioId: string, userId: string): Promise<AudioResponseDto> {
        try {
            ...
            this.logger.log(`STT 텍스트 업그레이드 시작: 오디오 ID=${audioId}, 텍스트 길이=${audioEntity.transcriptionText.length}`);
            ...
            try {
                ...
                this.logger.log(`텍스트 품질 향상 완료: 향상된 텍스트 길이=${sttResult.enhancedText.length}`);
                ...
                this.logger.error(`텍스트 품질 향상 실패: ${enhanceError.message}`, enhanceError.stack);
                ...
                throw new InternalServerErrorException(`텍스트 품질 향상 실패: ${enhanceError.message}`);
            }
     ...
     */
    async findOneWithStt(audioId: string, userId: string): Promise<AudioResponseDto> {
